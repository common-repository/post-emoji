...
use Emoji\Vendor\Symfony\Component\DependencyInjection\Config\AutowireServiceResource;
...
use Emoji\Vendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException;
 ...
 */
class AutowirePass extends \Emoji\Vendor\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public static function createResourceForClass(\ReflectionClass $reflectionClass)
    {
        ...
        return new \Emoji\Vendor\Symfony\Component\DependencyInjection\Config\AutowireServiceResource($reflectionClass->name, $reflectionClass->getFileName(), $metadata);
    }
    /**
     * {@inheritdoc}
     */
    protected function processValue($value, $isRoot = \false)
    {
        ...
        } catch (\Emoji\Vendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException $e) {
    ...
    private function doProcessValue($value, $isRoot = \false)
    {
        ...
        if (!$value instanceof \Emoji\Vendor\Symfony\Component\DependencyInjection\Definition || !$value->isAutowired() || $value->isAbstract() || !$value->getClass()) {
            return $value;
        }
        ...
        try {
            ...
            throw new \Emoji\Vendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $e->getMessage(), 0, $e);
        }
        if ($constructor) {
    ...
    private function autowireCalls(\ReflectionClass $reflectionClass, array $methodCalls)
    {
        foreach ($methodCalls as $i => $call) {
            ...
            if ($method instanceof \ReflectionFunctionAbstract) {
                ...
                $definition = new \Emoji\Vendor\Symfony\Component\DependencyInjection\Definition($reflectionClass->name);
                try {
    ...
    private function autowireMethod(\ReflectionFunctionAbstract $reflectionMethod, array $arguments)
    {
        ...
        foreach ($parameters as $index => $parameter) {
            ...
            $type = \Emoji\Vendor\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \true);
            if (!$type) {
                ...
                if (!$parameter->isDefaultValueAvailable()) {
                    ...
                    $type = \Emoji\Vendor\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \false);
                    $type = $type ? \sprintf('is type-hinted "%s"', $type) : 'has no type-hint';
                    throw new \Emoji\Vendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, \sprintf('Cannot autowire service "%s": argument "$%s" of method "%s()" %s, you should configure its value explicitly.', $this->currentId, $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method, $type));
                }
                // specifically pass the default value
                $arguments[$index] = $parameter->getDefaultValue();
                continue;
            }
            if (!($value = $this->getAutowiredReference($ref = new \Emoji\Vendor\Symfony\Component\DependencyInjection\TypedReference($type, $type, !$parameter->isOptional() ? $class : ''), 'for ' . \sprintf('argument "$%s" of method "%s()"', $parameter->name, $class . '::' . $method)))) {
                $failureMessage = $this->createTypeNotFoundMessage($ref, \sprintf('argument "$%s" of method "%s()"', $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method));
                if ($parameter->isDefaultValueAvailable()) {
                    $value = $parameter->getDefaultValue();
                } elseif (!$parameter->allowsNull()) {
                    throw new \Emoji\Vendor\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $failureMessage);
                }
     ...
     */
    private function getAutowiredReference(\Emoji\Vendor\Symfony\Component\DependencyInjection\TypedReference $reference, $deprecationMessage)
    {
        ...
        if (isset($this->definedTypes[$type])) {
            return new \Emoji\Vendor\Symfony\Component\DependencyInjection\TypedReference($this->types[$type], $type);
        }
        ...
        if (isset($this->autowired[$type])) {
            return $this->autowired[$type] ? new \Emoji\Vendor\Symfony\Component\DependencyInjection\TypedReference($this->autowired[$type], $type) : null;
        }
        if (!$this->strictMode) {
     ...
     * @param string $id
     */
    private function populateAvailableType($id, \Emoji\Vendor\Symfony\Component\DependencyInjection\Definition $definition, $onlyAutowiringTypes)
    {
    ...
    private function createAutowiredDefinition($type)
    {
        ...
        $argumentDefinition = new \Emoji\Vendor\Symfony\Component\DependencyInjection\Definition($type);
        ...
        return new \Emoji\Vendor\Symfony\Component\DependencyInjection\TypedReference($argumentId, $type);
    }
    private function createTypeNotFoundMessage(\Emoji\Vendor\Symfony\Component\DependencyInjection\TypedReference $reference, $label)
    {
        ...
        if (!$r) {
            ...
            try {
                $resource = new \Emoji\Vendor\Symfony\Component\Config\Resource\ClassExistenceResource($type, \false);
    ...
    private function createTypeAlternatives(\Emoji\Vendor\Symfony\Component\DependencyInjection\TypedReference $reference)
    {
