 ...
 */
namespace Emoji\Vendor\Symfony\Component\Config\Resource;

use Emoji\Vendor\Symfony\Component\DependencyInjection\ServiceSubscriberInterface;
use Emoji\Vendor\Symfony\Component\EventDispatcher\EventSubscriberInterface;
/**
 * @author Nicolas Grekas <p@tchwork.com>
 */
class ReflectionClassResource implements \Emoji\Vendor\Symfony\Component\Config\Resource\SelfCheckingResourceInterface, \Serializable
{
    private $files = [];
    private $className;
    ...
    private function generateSignature(\ReflectionClass $class)
    {
        ...
        if (\defined('Emoji\\Vendor\\HHVM_VERSION')) {
            foreach ($class->getMethods(\ReflectionMethod::IS_PUBLIC | \ReflectionMethod::IS_PROTECTED) as $m) {
                // workaround HHVM bug with variadics, see https://github.com/facebook/hhvm/issues/5762
                (yield \preg_replace('/^  @@.*/m', '', new \Emoji\Vendor\Symfony\Component\Config\Resource\ReflectionMethodHhvmWrapper($m->class, $m->name)));
            }
        } else {
            foreach ($class->getMethods(\ReflectionMethod::IS_PUBLIC | \ReflectionMethod::IS_PROTECTED) as $m) {
                $defaults = [];
        ...
        if (\interface_exists(\Emoji\Vendor\Symfony\Component\EventDispatcher\EventSubscriberInterface::class, \false) && $class->isSubclassOf(\Emoji\Vendor\Symfony\Component\EventDispatcher\EventSubscriberInterface::class)) {
            (yield \Emoji\Vendor\Symfony\Component\EventDispatcher\EventSubscriberInterface::class);
            (yield \print_r(\call_user_func([$class->name, 'getSubscribedEvents']), \true));
        }
        if (\interface_exists(\Emoji\Vendor\Symfony\Component\DependencyInjection\ServiceSubscriberInterface::class, \false) && $class->isSubclassOf(\Emoji\Vendor\Symfony\Component\DependencyInjection\ServiceSubscriberInterface::class)) {
            (yield \Emoji\Vendor\Symfony\Component\DependencyInjection\ServiceSubscriberInterface::class);
...
class ReflectionMethodHhvmWrapper extends \ReflectionMethod
{
    public function getParameters()
    {
        ...
        foreach (parent::getParameters() as $i => $p) {
            $params[] = new \Emoji\Vendor\Symfony\Component\Config\Resource\ReflectionParameterHhvmWrapper([$this->class, $this->name], $i);
        }
        return $params;
    }
}
